...
void main() 
{
	...
	v_Position = -u_ViewProjection[3].xyz * mat3(u_ViewProjection);
...
Ray CreateCameraRay(vec2 uv, vec3 M) 
{
    //vec3 origin = vec3(20.*sin(M.x*5.),2.,-30.*cos(M.x*5.) - 20.);
	vec3 origin = M;

	vec3 lookat = vec3(M.x, M.y, M.z - 20.); //0, 2. * M.y * 5. + 10., 20.);
...
void main() 
{
    ...
    //centerPosition = (vec2((v_Position.x),v_Position.y) / Input.Resolution.xy) * 2. -1.;
	//vec3 position = vec3(v_Position.x, v_Position.y * 0.02, v_Position.z + 20.);
	Ray ray = CreateCameraRay(uv, v_Position);
