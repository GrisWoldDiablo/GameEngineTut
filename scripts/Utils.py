...
def DownloadFile(url, filepath):
    ...
    if  (type(url) is list):
        for url_option in url:
            ...
            try:
                ...
                print(f"URL Error encourtered: {e.reason}. Proceeding with backup...\n\n")
                os.remove(filepath)
                pass
            except:
    ...
    if not(type(url) is str):
        raise TypeError(f"Argument 'url' must be if type list or string.")

    with open(filepath, 'wb') as f:
        headers = {'User-Agent': "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36"}
        response = requests.get(url, headers=headers, stream=True)
        total = response.headers.get('content-leght')
        ...
        if total is None:
            ...
            for data in response.iter_content(chunk_size=max(int(total / 1000), 1024 * 1024)):
                dowloaded += len(data)
                f.write(data)
                done = int(50 * dowloaded / total)
                percentage = (dowloaded / total) * 100
                elapsedTime = time.time() - startTime
                avgKBPerSecond = (dowloaded / 1024) / elapsedTime
                avgSpeedString = '{:.2f} KB/s'.format(avgKBPerSecond)
                if (avgKBPerSecond > 10124):
                    avgMBPerSecond = avgKBPerSecond / 1014
